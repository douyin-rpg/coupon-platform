...
export async function GET(request: NextRequest) {
  ...
  const { data: user, error } = await supabase
    .from("users")
    .select("id, username, real_name, balance, verify_status, bank_bound, bank_account_name, bank_card_number, bank_name, payment_password_hash, id_card, id_card_name, id_card_front, id_card_back, verify_rejected_reason, credit_score, login_frozen, funds_frozen")
    .eq("id", userId)
    .single();
  ...
  return NextResponse.json({
    ...
    loginFrozen: user.login_frozen || false,
    fundsFrozen: user.funds_frozen || false,
  });
}
