...
export default function AdminBannersPage() {
  ...
  return (
    <div className="p-6 md:p-8">
          ...
          <Button className="bg-[#1890FF] hover:bg-[#1890FF]/80 text-white" onClick={() => { setImageUrl(''); setLinkUrl(''); setTitle(''); setSortOrder('0'); setAddOpen(true); }}>
            新增轮播图
          </Button>
        </div>
      ...
      <Dialog open={addOpen} onOpenChange={setAddOpen}>
        <DialogContent className="sm:max-w-lg">
          ...
          <div className="space-y-4 py-2">
            ...
            <Button className="w-full bg-[#1890FF] hover:bg-[#1890FF]/80 text-white" onClick={handleAdd} disabled={loading || !imageUrl}>
              {loading ? '创建中...' : '创建轮播图'}
            </Button>
          </div>
        </DialogContent>
